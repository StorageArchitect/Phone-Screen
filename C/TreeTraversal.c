...
void recursePreOrder(Node *tree) {
  //Preorder (Root, Left, Right)
  if(tree == NULL){
    return;
  }
  printf("%s\n",((Node *) tree)->data);
  recursePreOrder((Node *) tree->left);
  recursePreOrder((Node *) tree->right);

}

void recursePostOrder(Node *tree) {
  //Postorder (Left, Right, Root)
  if(tree == NULL){
    return;
  }
  recursePostOrder((Node *) tree->left);
  recursePostOrder((Node *) tree->right);
  printf("%s\n",((Node *) tree)->data);

}

void recurseInOrder(Node *tree) {
  // Inorder (Left, Root, Right)
  if(tree == NULL){
    return;
  }
  recurseInOrder((Node *) tree->left);
  printf("%s\n",((Node *) tree)->data);
  recurseInOrder((Node *) tree->right);
}

void traverseDFSOrder(Node *tree) {
  printf("Traversal Pre Order \n \n");
  recursePreOrder(tree);
  printf("Traversal Post Order \n \n");
  recursePostOrder(tree);
